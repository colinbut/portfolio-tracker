package com.mycompany.portfolio_tracker.util;
 ...
 * href="http://www.tickertech.com"> <tt>http://www.tickertech.com</tt> </a> <Br>
...
public class StrathQuoteServer {
	...
	protected static final String _URL_NAME = "http://www.tickertech.com/cgi/?a=profile&ticker=";
	 ...
	 *           The amount returned may contain commas, for example, "2,243.87" <br>
	 */
	...
	public static String getCompanyName(String tickerSymbol)
			throws WebsiteDataException, NoSuchTickerException {
		_TOKEN1 = _TOKEN1a + tickerSymbol.toUpperCase() + _TOKEN1b;
		String strURLStart = _URL_NAME;
		URL urlWebPage = null;
		InputStreamReader isr = null;
		BufferedReader brWebPage = null;
		...
		try {
			...
			while (true) {
				...
				if (strLine.indexOf("<b>Company Name:</b>") != -1) {
					System.out.println("found token");
					// brWebPage.readLine();
					strLine = brWebPage.readLine(); // move down two lines
					break;
		...
		// find the stock quote in the line
		int a = strLine.indexOf("<font color");
		int b = strLine.indexOf("</font>");
		String strStockValue = strLine.substring(a + 20, b);
